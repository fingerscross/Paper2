...
# 5、GNN 模型 使用稀疏图注意力网络（SpGAT）作为扩散过程的预测模型
...
# 重构损失：衡量 VAE 对输入数据的重构质量。
# 前向传播损失：衡量 GNN 对扩散预测的准确性。
...
for epoch in range(600):
    ...
    for batch_idx, data_pair in enumerate(train_loader):
        ...
        for i, x_i in enumerate(x):
            ...
            # sklearn自带函数 计算预测种子集合与真实种子集合的精确率和召回率，一个批次中累加
...
# 从训练好的vae_model中提取编码器和解码器 即种子集->z z->种子集
encoder = vae_model.Encoder
decoder = vae_model.Decoder

# 定义逆问题的损失函数
# 使用均方误差 (MSE) 衡量预测扩散结果 y_hat 和y_true的误差
# 对种子集合的稀疏性进行约束，鼓励生成的种子集合尽可能稀疏（选的种子节点少）
...
z_hat = 0
# 将选出的种子集合通过编码器 encoder 映射到潜在空间
# 并对多个种子集合z取平均，得到初始的 z_hat
for i in topk_seed:
...
# Adam 优化器对z_hat进行优化，学习更优的潜在表示。
z_optimizer = Adam([z_hat], lr=1e-4)

for i in range(300):
    # 解码器将z_hat映射回中自己x_hat
    x_hat = decoder(z_hat)
    # 使用 训练好的图神经网络扩散模型forward_model计算种子集在图上的扩散效果
    y_hat = forward_model(x_hat.squeeze(0).unsqueeze(-1), adj)
    # 二值化扩散效果，感染or未感染
    y = torch.where(y_hat > 0.05, 1, 0)
    # 计算损失
    loss, L0 = loss_inverse(y_true, y_hat, x_hat)
    # 反向传播更新z_hat，优化种子集合潜在表示z
    loss.backward()
    z_optimizer.step()
